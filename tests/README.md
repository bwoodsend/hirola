# Welcome to hoatzin's test suite!
...
hoatzin aims to be out-the-box compatible with
...
that PyInstaller does need special help when compiling hoatzin
then that *help* goes in a *hook* file (see
`hoatzin/__pyinstaller/hook-hoatzin.py`).
...
changed the hoatzin source code.
...
This executable is just `pytest.main()` with hoatzin
...
Otherwise you may unintentionally use the original source hoatzin
...
If you've modified either the source code or the hooks for hoatzin
